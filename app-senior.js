...
async function generatePDF(){
  ...
  doc.text('SOLICITUD ASOCIADO JR SENIOR 25 A 29 AÑOS',119,35,{align:'center'});
  ...
  let y=83;
  const p1=`Solicito sea admitido(a) mi hijo(a) ${hijo}, quien nació el día ${nac}, en la Categoría de “ASOCIADO JR SENIOR” y como tal continúe haciendo uso de las instalaciones del CLUB ATLAS CHAPALITA, en tanto no cumpla los 30 años y permanezca soltero(a).`;
  const p2='Al mismo tiempo, acepto que la cuota para asociado JUNIOR, que para este caso es del 30% (Treinta) de la mensualidad vigente, sea incluida automáticamente en mi estado de cuenta, en la inteligencia que tengo el derecho en cualquier momento de renunciar a esta categoría, mediante simple aviso por escrito al CLUB y/o entrega de credencial de asociado, por ende se dejará de cobrar a partir del siguiente mes.';
  ...
  doc.text('ASOCIADO(A) JR SENIOR',98.5,sigY+5,{align:'center'});
  doc.text('CLUB ATLAS CHAPALITA',171.5,sigY+5,{align:'center'});

  doc.setFont('helvetica','normal');doc.setFontSize(9.8);
  doc.text('Celular',18,sigY+20);doc.text($('#celular').value.trim(),36,sigY+20);doc.line(34,sigY+21,116,sigY+21);
  doc.text('Correo electrónico',18,sigY+30);doc.text($('#correo').value.trim(),52,sigY+30);doc.line(50,sigY+31,116,sigY+31);

  const safe=hijo.replace(/[^a-zA-Z0-9áéíóúÁÉÍÓÚñÑ]+/g,'_');
  doc.save(`Solicitud_JR_Senior_${safe||'Asociado'}.pdf`);
